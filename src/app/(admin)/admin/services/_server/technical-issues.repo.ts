...
import { listAppearanceIssueOptions } from "./technical_catalog_bridge.repo";
...
export async function getPanel(serviceRequestId: string) {
    ...
    return {
        ...
        catalogs: {
            ...
            vendors,
            appearanceIssues: {
                CASE: appearanceIssues.CASE.map((x) => ({
                    ...x,
                    deductionScore: Number(x.deductionScore ?? 0),
                })),
                CRYSTAL: appearanceIssues.CRYSTAL.map((x) => ({
                    ...x,
                    deductionScore: Number(x.deductionScore ?? 0),
                })),
                DIAL: appearanceIssues.DIAL.map((x) => ({
                    ...x,
                    deductionScore: Number(x.deductionScore ?? 0),
                })),
            },

...
export default function TechnicalIssuesInlineWorkbench({
    ...
    async function confirmIssue(id: string) {
        const res = await fetch(`/api/admin/technical-issues/${id}/confirm`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({}),
        });
        ...
        const json = await res.json().catch(() => ({}));
        if (!res.ok) {
            alert(json?.error || "Không thể xác nhận issue");
            return;
        }
                        ...
                        <div key={issue.id} className="rounded-2xl border border-slate-200 bg-white p-4">
                            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {!issue.isConfirmed && issue.executionStatus === "OPEN" && (
                                        <button
                                            type="button"
                                    ...
                                    {issue.isConfirmed && issue.executionStatus === "IN_PROGRESS" && (
                                        <>
                                            <button
                                                type="button"

...
function UiField({ label, children }: { label: string; children: any }) {
    return (
...
function UiButton({ children, variant = "outline", className = "", ...props }: any) {
    ...
    return (
        <button
            {...props}
            className={`inline-flex h-10 items-center justify-center rounded-xl border px-4 text-sm font-medium transition disabled:cursor-not-allowed disabled:opacity-50 ${styles[variant] || styles.outline} ${className}`}
        >
            {children}
        </button>
    );
...
function UiCard({ title, subtitle, children, right }: { title?: string; subtitle?: string; children: any; right?: any }) {
    return (
...
export default function AdminProductListPageClient(props: PageProps) {
    ...
    return (
        <div className="space-y-5">
                    ...
                    <div className="space-y-4">
                        <div className="inline-flex w-fit rounded-xl border border-slate-200 bg-slate-100 p-1">
                            <button
                                ...
                                className={`inline-flex items-center gap-2 rounded-lg px-3.5 py-2 text-sm font-medium transition ${!isStrapCatalog
                                    ? "border border-slate-300 bg-white text-slate-950 shadow-sm"
                                    : "text-slate-600 hover:text-slate-900"
                                    }`}
                            >
                                Sản phẩm
                                <span className={`rounded-full px-2 py-0.5 text-xs ${!isStrapCatalog ? "bg-slate-900 text-white" : "bg-white text-slate-600"}`}>{counts.all}</span>
                            </button>
                        ...
                        <div className="space-y-1">
                            ...
                            <p className="text-sm text-slate-500">
                                Cùng hệ ngôn ngữ UI với technical assessment: gọn, rõ trạng thái và ưu tiên thao tác.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 self-start">
                        {!isStrapCatalog && (
                            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-right shadow-sm">
                                <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Đã chọn</div>
                                <div className="mt-1 text-2xl font-semibold leading-none text-slate-950">{selectedIds.length}</div>
                            </div>
                        )}
            ...
            <UiCard title="Phân loại danh sách" subtitle="Giữ phần điều hướng ở mức gọn, không chiếm sự chú ý của bảng dữ liệu.">
                <div className="flex flex-wrap gap-2">
                    {segmentTabs.map((tab) => {
                        ...
                        return (
                            <button
                                key={tab.key}
                                type="button"
                                onClick={() => setView(tab.key as string)}
                                className={`inline-flex items-center gap-2 rounded-xl border px-3.5 py-2.5 text-sm font-medium transition ${active
            ...
            <UiCard
                title="Bộ lọc"
                subtitle="Tập trung thao tác lọc nhanh giống workbench của technical assessment."
                right={!isStrapCatalog ? <div className="text-sm text-slate-500">Đã chọn: <span className="font-semibold text-slate-900">{selectedIds.length}</span></div> : null}
            >
                <form
                    className="space-y-4"
                    onSubmit={(e) => {
                ...
                >
                    <div className={`grid grid-cols-1 gap-4 ${isStrapCatalog ? "lg:grid-cols-5" : "lg:grid-cols-7"}`}>
                        <UiField label="Tìm kiếm">
                            <UiInput
                                value={formQ}
                                onChange={(e: any) => setFormQ(e.target.value)}
                        ...
                        {!isStrapCatalog && (
                            <>
                                <UiField label="Type">
                                    <UiSelect value={formType} onChange={(e: any) => setFormType(e.target.value)}>
                                ...
                                <UiField label="Brand">
                                    <UiSelect value={formBrandId} onChange={(e: any) => setFormBrandId(e.target.value)}>
                                        <option value="">(All)</option>
                                        {props.brands.map((b) => (
                        ...
                        <UiField label="Vendor">
                            <UiSelect value={formVendorId} onChange={(e: any) => setFormVendorId(e.target.value)}>
                                <option value="">(All)</option>
                                {props.vendors.map((v) => (
                        ...
                        <UiField label="Image">
                            <UiSelect value={formHasImages} onChange={(e: any) => setFormHasImages(e.target.value)}>
                                <option value="">(All)</option>
                        ...
                        <UiField label="Sắp xếp">
                            <UiSelect value={formSort} onChange={(e: any) => setFormSort(e.target.value)}>
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        <UiButton type="submit" variant="primary">Lọc</UiButton>
                        <UiButton type="button" onClick={clearFilters}>Clear</UiButton>
                    </div>
                </form>
            </UiCard>
                ...
                <UiCard>
                    <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-sm text-slate-500">Thao tác nhanh theo cùng pattern với technical assessment.</span>
                        </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4 backdrop-blur-sm">
                    <div className="w-full max-w-md space-y-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl">
                        ...
                        <div className="flex justify-end gap-2 pt-3">
                            <button
                                className="inline-flex h-10 items-center justify-center rounded-xl border border-slate-200 px-4 text-sm font-medium text-slate-900 hover:bg-slate-50"
                            ...
                            <button
                                className="inline-flex h-10 items-center justify-center rounded-xl border border-slate-900 bg-slate-900 px-4 text-sm font-medium text-white hover:bg-slate-800"
                                onClick={async () => {
            ...
            <UiCard title={isStrapCatalog ? "Danh sách dây" : "Danh sách sản phẩm"} subtitle={isStrapCatalog ? "Bố cục bảng đồng bộ với technical assessment: thông tin chính rõ, spacing thoáng." : "Bảng dữ liệu là vùng ưu tiên chính, giảm tối đa cảm giác dashboard ở phần đầu."}>
                        ...
                        <table className="min-w-full text-sm">
                            ...
                            <tbody>
                                {rows.length === 0 ? (
                                    ...
                                    rows.map((p) => (
                                        <tr key={p.id} className="border-t border-slate-100 transition hover:bg-slate-50/70 [&>td]:align-middle">
                                    ...
                                    rows.map((p) => {
                                        ...
                                        return (
                                            <tr key={p.id} className="border-t border-slate-100 transition hover:bg-slate-50/70 [&>td]:align-middle">
                                                ...
                                                <td className="px-3 py-5">
                                                    <div className="space-y-1">
                                                        <div className="text-sm font-medium leading-5 text-slate-950">{p.title || "-"}</div>
            ...
            <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600 shadow-sm">
                ...
                <div className="flex items-center gap-2">
                    <button
                        type="button"
                        className="inline-flex h-10 items-center justify-center rounded-xl border border-slate-200 px-4 font-medium text-slate-900 transition hover:bg-slate-50 disabled:opacity-50"
